...
import { BTCInfoHandler, marketAllAPI, toDisplayMarket } from "@/entities";
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (type === "list") {
      const BTCMarkets = await marketAllAPI();
      const krwMarkets = BTCMarkets.filter((market) => market.market === "KRW-BTC").map((m) => ({
        ...m,
        market: toDisplayMarket(m.market),
      }));
      return NextResponse.json({ success: true, data: krwMarkets }, { status: 200 });
    }

    // 기본: 마켓 정보 + 시세
    const data = await BTCInfoHandler();
